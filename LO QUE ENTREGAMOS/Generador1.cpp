...
int main(){
	...
	for (int w=0;w<2000;w++){
		...
		int trabajos= rand() % (10*10*10) + 1;
		//random entre 1 y 10*10*10 la cantidad de trabajos
		
		vector<vector<int>> precios (trabajos);
		for(int i =0; i<trabajos; ++i){
			for (int j=0; j<i+1; ++j){
				int x = rand() % (10*10*10*10*10*10);
				//random entre 0 y 10*10*10*10*10*10 el precio
		...
		ofstream myfile;
		myfile.open ("1.csv", std::ios_base::app);
		//myfile << "Trabajos,CantEnMaquina,Ticks,Resultado,Caso\n";
		...
		myfile << trabajos << "," << actuallista[posicion].size() << "," << tiempo << "," << costo << "," << "r8" << "\n";
		myfile.close();
	}
