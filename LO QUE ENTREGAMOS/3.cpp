...
tuple<vector<vector<int>>, int, vector<vector<int>>> PrimModificado(vector<vector<int>> & matrizAdy, int n, int f) {
	...
	// como es un arbol se que la cantidad de rutas elegidas es menor que la cantidad de clientes
...
int main(){
	...
	while( f!=0){
		...
		cout << costoDelG << " ";
		cout << c << " ";
		//Notar que siempre la cantidad de aristas que utilizamos es la cantidad de clientes (explicado en informe).
		//es porque hacemos c iteraciones en la que cada una agregamos una arista exactamente.
